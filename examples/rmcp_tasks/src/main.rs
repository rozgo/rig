...
//! optional`). The example opts into `Preferred` MCP dispatch and
//! `ContinueTurns` agent completion, so the model immediately receives the
//! server's `model-immediate-response` placeholder, keeps working, and the
//! real result is injected into a later turn as a labeled "Background task
//! update" the final answer incorporates.
//!
...
use rig::agent::{
    AgentHook, HookContext, StepEventKind, TaskCompletionPolicy, ToolTaskResultAction,
...
use rig::tool::rmcp::{MODEL_IMMEDIATE_RESPONSE_META_KEY, McpClientHandler, McpTaskPolicy};
use rig::tool::server::ToolServer;
use rmcp::model::{
    CallToolRequestParams, CallToolResult, CancelTaskParams, CancelTaskResult, ClientInfo,
    ContentBlock, CreateTaskResult, ErrorData, GetTaskParams, GetTaskPayloadParams,
    GetTaskPayloadResult, GetTaskResult, Implementation, ListToolsResult, Meta,
    PaginatedRequestParams, ProtocolVersion, ServerCapabilities, ServerInfo, ServerNotification,
    Task, TaskStatus, TaskStatusNotification, TaskStatusNotificationParam, TaskSupport,
...
impl<M: CompletionModel> AgentHook<M> for TaskTracer {
    async fn on_tool_task_started(
        ...
        println!(
            "⏳ task {} started for tool `{}`",
            event.task_id, event.tool_name
        );
    ...
    async fn on_tool_task_status(
        ...
        println!("   task {} is {}", event.task_id, event.status);
...
async fn main() -> anyhow::Result<()> {
    ...
    // Register the server's tools; this example opts optional-task tools into
    // task-augmented dispatch, and the handler's notification registry wakes
    // waiting handles on status pushes.
