...
//!
//! A tool dispatched through Rig's canonical erased-tool boundary resolves to
//! a [`ToolDispatch`]: either an already-final [`ToolResult`] (every ordinary
//! tool), or a
...
//!
//! Dropping a [`ToolTaskHandle`] (after dropping any future borrowing it), or
//! dropping the future returned by [`wait`](ToolTaskHandle::wait), abandons the
//! task **locally only** — the backend keeps running it.
//! [`cancel`](ToolTaskHandle::cancel) is the real, best-effort cancellation.
//!
//! # Durability
//!
...
use super::{ToolContext, ToolExecutionError, ToolOutput, ToolResult};
...
/// [`ToolTaskHandle`]. Timestamps are the backend's own ISO-8601 strings and
/// are carried verbatim. Rig does not derive a local monotonic deadline from
/// them; a driver applies its configured time budget when it starts driving a
/// fresh or rehydrated handle.
...
/// # Contract
///
/// - [`wait`](Self::wait) always resolves to a [`ToolTaskResult`], whose
///   canonical [`ToolResult`] keeps execution failures classified instead of
///   returning them as a bare error. It borrows the handle so callers can
///   still poll or cancel while result retrieval is pending.
/// - Drive at most one `wait` future for a handle at a time. Status polling and
///   cancellation may run concurrently with that future.
...
///   transport/protocol problems represented as [`ToolExecutionError`] so
///   policies can act on their normalized kind.
...
/// Terminal output and host-only metadata from a deferred tool task.
///
/// This mirrors Rig's canonical inline execution boundary: [`result`](Self::result)
/// is the structured model-visible outcome, while [`context`](Self::context)
/// carries typed metadata that hooks and hosts may inspect but models never see.
#[derive(Debug)]
...
impl ToolTaskResult {
    /// Construct a successful terminal task result.
    ...
    /// Construct a failed terminal task result.
    ...
    /// Construct a cancelled terminal task result.
    pub fn cancelled(error: ToolExecutionError) -> Self {
        Self {
    ...
    /// Attach the typed host-only context produced while retrieving the result.
    ...
    /// Terminal lifecycle status associated with the result.
    pub fn status(&self) -> ToolTaskStatus {
        self.status
    }

    /// Host-only typed result metadata.
    pub fn context(&self) -> &ToolContext {
        &self.context
    }

    /// Mutably access host-only metadata while a protocol adapter assembles
    /// the terminal result.
    pub fn context_mut(&mut self) -> &mut ToolContext {
        &mut self.context
    }

    pub(crate) fn into_parts(self) -> (ToolResult, ToolContext) {
...
impl ToolDispatch {
    ...
    pub async fn resolve(self) -> ToolTaskResult {
        match self {
            ToolDispatch::Completed(result) => {
...
// loop, so on native targets it must stay `Send + Sync`. This fails to compile
// if a future change drops the property.
#[cfg(not(target_family = "wasm"))]
const _: fn() = || {
    fn assert_send_sync<T: Send + Sync>() {}
    assert_send_sync::<ToolDispatch>();
    assert_send_sync::<ToolTaskResult>();
...
mod tests {
    ...
    impl ToolTaskHandle for StubHandle {
        ...
        fn wait(&self) -> WasmBoxedFuture<'_, ToolTaskResult> {
            Box::pin(async move { ToolTaskResult::success(ToolOutput::text(&self.output)) })
        }
    ...
    async fn dispatch_resolve_awaits_deferred_handles() {
        let completed = ToolDispatch::Completed(ToolResult::success(ToolOutput::text("done")));
        assert_eq!(completed.resolve().await.result().output().render(), "done");
        ...
        assert_eq!(
            deferred.resolve().await.result().output().render(),
            "task output"
        );
    }
}
