...
use rmcp::model::{
    ...
    ElicitRequestParams, ElicitResult, ElicitationAction, ErrorData, GetTaskParams,
    GetTaskPayloadParams, GetTaskPayloadResult, GetTaskResult, Implementation, ListToolsResult,
    Meta, PaginatedRequestParams, ProtocolVersion, RelatedTaskMetadata, RequestParamsMeta,
...
async fn main() -> anyhow::Result<()> {
    ...
    let response = openai_client
        ...
        .max_turns(6)
        .task_deadline(std::time::Duration::from_secs(120))
        .run()
        .await?;
