...
//! refresh and the task-status wakeup happen first — so a delegate can never
//! break rig invariants. Every method defaults to a no-op; implement only
//! what you consume.
//!
...
mod tests {
    ...
    async fn delegate_receives_forwarded_notifications() {
        ...
        // Notifications are processed asynchronously by the client service.
