...
use crate::{
    ...
    tool::{ToolTaskDescriptor, ToolTaskStatus},
};
...
pub enum TaskCompletionPolicy {
    ...
    /// one user message, in the order the driver settled them) only after
    /// every deferred handle resolves. The model only ever sees real results;
    /// message shapes are identical to a run without tasks.
    JoinTurn,
    ...
    /// run; the terminal result is injected into a later turn's prompt as a
    /// labeled text part. The model keeps working while the task runs.
    #[default]
    ContinueTurns,
}
...
pub struct TaskResolution {
    ...
    /// Whether `output` is synthetic (a hook rewrite, cancellation reason, or
    /// deadline text — delivered verbatim) rather than real tool output
    /// (parsed as possibly-multimodal). Mirrors the inline tool path's
    /// synthetic-result handling.
    pub synthetic: bool,
}

impl TaskResolution {
    /// A resolution carrying real tool output.
    pub fn new(output: impl Into<String>, status: ToolTaskStatus) -> Self {
        Self {
            output: output.into(),
            status,
            synthetic: false,
    ...
    /// A resolution carrying synthetic text (delivered to the model verbatim).
    pub fn synthetic(output: impl Into<String>, status: ToolTaskStatus) -> Self {
        Self {
            output: output.into(),
            status,
            synthetic: true,
...
/// into the tool_result content filling its held batch slot: synthetic text is
/// delivered verbatim, real output is parsed as possibly-multimodal — the same
/// split the inline tool path applies.
fn task_slot_content(task: &PendingTask, resolution: &TaskResolution) -> UserContent {
    ...
    if resolution.synthetic {
        tool_result_message(id, call_id, resolution.output.clone())
    } else {
        tool_result_output(id, call_id, resolution.output.clone())
    }
}

/// The labeled text part injected into a later turn's prompt when a
/// [`ContinueTurns`](TaskCompletionPolicy::ContinueTurns) task resolves.
pub(crate) fn task_result_notice_text(task: &PendingTask, resolution: &TaskResolution) -> String {
...
struct AwaitingTasksState {
    /// The batch's calls, exactly as they were pending in `ExecutingTools`
    /// (empty on the drain path).
    calls: Vec<PendingToolCall>,
    /// Result slots in the driver's settle order; `None` = owned by a pending
    /// join task (empty on the drain path).
...
impl AgentRun {
    ...
    ///   slot; when the last slot fills, the batch commits in settle order.
    ...
    ///   labeled notice for the next [`AgentRunStep::CallModel`] prompt —
    ...
    fn inject_task_notices(&mut self) {
        ...
        let notices: Vec<UserContent> = self
            ...
            .drain(..)
            .map(|(task, resolution)| {
                UserContent::text(task_result_notice_text(&task, &resolution))
            })
            .collect();
    ...
    fn finalize_or_drain(&mut self, response: PromptResponse) -> Result<AgentRunStep, PromptError> {
        ...
        match self.task_drain {
            ...
            TaskDrainPolicy::WaitAndResume if self.current_turn <= self.max_turns + 1 => {
...
mod task_tests {
    use super::*;
    use crate::message::{Text, ToolFunction, ToolResultContent};
    use serde_json::json;
    ...
    fn join_turn_deferral_holds_batch_until_all_tasks_resolve() {
        ...
        // The commit is one user message in settle order: inline result first.
    ...
    fn continue_turns_resolution_injects_notice_into_next_call_model() {
        let mut run = run_to_tools(vec![tool_call("call_1", "work")], 3);
        let deferred = task("call_1", "task-1", TaskCompletionPolicy::ContinueTurns, 1);
        let expected_notice = task_result_notice_text(
            &deferred,
            &TaskResolution::new("the answer is 42", ToolTaskStatus::Completed),
        );
        run.tool_batch_results(vec![ToolCallResolution::Deferred(Box::new(deferred))])
            .expect("batch accepted");
    ...
    fn waiting_on_tasks_consumes_no_turns() {
        // max_turns(1) allows exactly two model calls; holding a join batch
        // open must not eat into that budget.
        let mut run = run_to_tools(vec![tool_call("call_1", "work")], 1);
    ...
    fn done_with_pending_tasks_wait_and_resume_runs_one_extra_turn() {
        ...
        let UserContent::Text(Text { text, .. }) = parts[0] else {
            panic!("the drained notice must be the prompt");
        };
        assert!(text.contains("late result"));
    ...
    fn done_with_exhausted_budget_degrades_to_finish_with_unresolved() {
        // max_turns(0): the tool turn plus the final answer exhaust the budget,
        // so the default WaitAndResume drain degrades instead of erroring.
        let mut run = run_to_tools(vec![tool_call("call_1", "work")], 0);
        run.tool_batch_results(vec![ToolCallResolution::Deferred(Box::new(task(
            "call_1",
            "task-1",
            TaskCompletionPolicy::ContinueTurns,
    ...
    fn notice_texts_are_pinned() {
        ...
        assert_eq!(
            task_result_notice_text(
                &pending,
                &TaskResolution::new("42", ToolTaskStatus::Completed)
            ),
            "Background task update: tool `work` (task `task-9`, launched on turn 2 via tool \
             call `call_1`) completed.\nResult:\n42",
        );
        assert_eq!(
            task_result_notice_text(
                &pending,
                &TaskResolution::synthetic("deadline exceeded", ToolTaskStatus::Cancelled)
            ),
            "Background task update: tool `work` (task `task-9`, launched on turn 2 via tool \
             call `call_1`) was cancelled.\nResult:\ndeadline exceeded",
        );
    }
}
